...
def get_qs_data(persist="disk"):
    url = "https://github.com/1952154232/cloud/blob/main/qs_ranking_iso.xlsx"
    return get_data(url)
...
with col1:
    fig = px.choropleth(df_country_num,
                        locations='iso',  # 使用 ISO 代码作为位置
                        locationmode='ISO-3',  # 指定位置模式为 ISO 3166-1 alpha-3
                        color='num',  # 使用大学数量作为颜色编码
...
with col4:
    ...
    df_school = df_school[
        ['大学名称', '学术声誉', '雇主声誉', '师生比', '每位教员引用率', '国际教师占比', '国际学生占比', '国际研究网络',
         '就业结果', '可持续性']]
...
st.markdown('__数据来源:__ _QS Top Universities2024_')
st.markdown('__学生:__ 胡世恒 __指导老师__: 周辉 __')
